class Admin::FilmController < Admin::AuthenticatedController
  ...
  def create
    ...
    if @film.save
      flash[:notice] = "Película #{@film.title} ha sido creada con éxito."
  ...
  def update
    ...
    if @film.update_attributes(film_params)
      flash[:notice] = "Película #{@film.title} ha sido actualizada con éxito."
  ...
  def destroy
    @film = Film.find(params[:id])
    @film.destroy
    flash[:notice] = "Película #{@film.title} eliminada con éxito."
    redirect_to :action => 'index'
  end
  ...
  def index
    ...
    @page_title = 'Mostrando películas'
  end
