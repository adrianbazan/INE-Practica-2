...
class Admin::FilmControllerTest < ActionController::TestCase
  ...
  test "create" do
    ...
    assert_equal 'Film Pro Rails E-Commerce correctamente creado.', flash[:notice]
  end
  ...
  test "test_destroy" do
    assert_difference(Film, :count, -1) do
      post :destroy, :id => 1
      assert_equal flash[:notice], 'Film Pro Rails E-Commerce correctamente eliminado.'
      assert_response :redirect
      assert_redirected_to :action => 'index'
      get :index
      assert_response :success
      assert_tag :tag => 'div', :attributes => {:id => 'notice'},
        :content => 'Film Pro Rails E-Commerce correctamente eliminado.'
    end
  end
